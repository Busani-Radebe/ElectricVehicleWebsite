import React from 'react';
import './Background.css';
import video1 from 'C:/Users/Busan/OneDrive/Desktop/React Web/EV-website/src/assets/video1.mp4';
import image1 from 'C:/Users/Busan/OneDrive/Desktop/React Web/EV-website/src/assets/image1.png';
import image2 from 'C:/Users/Busan/OneDrive/Desktop/React Web/EV-website/src/assets/image2.png';
import image3 from 'C:/Users/Busan/OneDrive/Desktop/React Web/EV-website/src/assets/image3.png';

const Background = ({ playStatus, heroCount }) => {
    if (playStatus) {
        return (
            <video className='background fade-in' autoPlay loop muted>
                <source src={video1} type='video/mp4' />
            </video>
        );
    } else if (heroCount === 0) {
        return <img src={image1} className='background fade-in' alt='Background Image' />;
    } else if (heroCount === 1) {
        return <img src={image2} className='background fade-in' alt='Background Image' />;
    } else if (heroCount === 2) {
        return <img src={image3} className='background fade-in' alt='Background Image' />;
    } else {
        // Return a default background or null if heroCount is not within expected range
        return null;
    }
};

export default Background;
